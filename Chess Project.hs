...
getPieceN (col,row) [] (y:ys) = if y == N(col,row) then "NB"             
								else getPieceN (col,row) [] ys
getPieceN (col,row) (x:xs) [] = if x == N(col,row) then "NW"                  
								else getPieceN (col,row) xs []
getPieceN (col,row) (x:xs) (y:ys) = if x == N(col,row) then "NW" 
                                else if y == N(col,row) then "NB"
								else getPieceN (col,row) xs ys
getPieceR _ [] []=""

getPieceR (col,row) [] (y:ys) = if y == R(col,row) then "RB"             
								else getPieceR (col,row) [] ys
getPieceR (col,row) (x:xs) [] = if x == R(col,row) then "RW"                  
								else getPieceR (col,row) xs []																
getPieceR (col,row) (x:xs) (y:ys) = if x == R(col,row) then "RW" 
                                else if y == R(col,row) then "RB"
								else getPieceR (col,row) xs ys
getPieceB _ [] []=""
	
getPieceB (col,row) [] (y:ys) = if y == B(col,row) then "BB"             
								else getPieceB (col,row) [] ys
getPieceB (col,row) (x:xs) [] = if x == B(col,row) then "BW"                  
								else getPieceB (col,row) xs []																
getPieceB (col,row) (x:xs) (y:ys) = if x == B(col,row) then "BW" 
                                else if y == B(col,row) then "BB"
								else getPieceB (col,row) xs ys
getPieceK _ [] []=""

getPieceK (col,row) [] (y:ys) = if y == K(col,row) then "KB"             
								else getPieceK (col,row) [] ys
getPieceK (col,row) (x:xs) [] = if x == K(col,row) then "KW"                  
								else getPieceK (col,row) xs []									
getPieceK (col,row) (x:xs) (y:ys) = if x == K(col,row) then "KW" 
                                else if y == K(col,row) then "KB"
								else getPieceK (col,row) xs ys	
getPieceQ _ [] []=""

getPieceQ (col,row) [] (y:ys) = if y == Q(col,row) then "QB"             
								else getPieceQ (col,row) [] ys
getPieceQ (col,row) (x:xs) [] = if x == Q(col,row) then "QW"                  
								else getPieceQ (col,row) xs []									
getPieceQ (col,row) (x:xs) (y:ys) = if x == Q(col,row) then "QW" 
                                else if y == Q(col,row) then "QB"
								else getPieceQ (col,row) xs ys	
getPieceP _ [] []=""
getPieceP (col,row) [] (y:ys) = if y == P(col,row) then "PB"             
								else getPieceP (col,row) [] ys
getPieceP (col,row) (x:xs) [] = if x == P(col,row) then "PW"                  
								else getPieceP (col,row) xs []								
getPieceP (col,row) (x:xs) (y:ys) = if x == P(col,row) then "PW" 
                                else if y == P(col,row) then "PB"
								else getPieceP (col,row) xs ys
								
getPiece _ [] [] = ""
							
getPiece (col,row) (x:xs) (y:ys) = getPieceP (col,row) (x:xs) (y:ys)  								
							++   getPieceQ (col,row) (x:xs) (y:ys)  
							++   getPieceK (col,row) (x:xs) (y:ys)  
							++   getPieceB (col,row) (x:xs) (y:ys)  
							++   getPieceN (col,row) (x:xs) (y:ys)  
							++   getPieceR (col,row) (x:xs) (y:ys) 
getPiece (col,row) [] (y:ys) =   getPieceP (col,row) [] (y:ys)  								
							++   getPieceQ (col,row) [] (y:ys)  
							++   getPieceK (col,row) [] (y:ys)  
							++   getPieceB (col,row) [] (y:ys)  
							++   getPieceN (col,row) [] (y:ys)  
							++   getPieceR (col,row) [] (y:ys)  
							
getPiece (col,row) (x:xs) [] = getPieceP (col,row) (x:xs) []  								
							++   getPieceQ (col,row) (x:xs) [] 
							++   getPieceK (col,row) (x:xs) []  
							++   getPieceB (col,row) (x:xs) []  
							++   getPieceN (col,row) (x:xs) []  
							++   getPieceR (col,row) (x:xs) []  
														
							
toString x= if x =="" then "    " else " " ++x++" "

-- I checked every location on the chess board and updates to String
visualizeBoard (color, w,b) = 
       do
	   putStrLn  ("    a    b    c    d    e    f    g    h")      
	   putStrLn  ("8" ++ "|" ++ toString(getPiece ('a',8) w b) ++ "|" ++ toString(getPiece ('b',8) w b) ++ "|" ++ toString(getPiece ('c',8) w b) ++ "|" ++ toString(getPiece ('d',8) w b) ++ "|" ++ toString(getPiece ('e',8) w b) ++ "|" ++ toString(getPiece ('f',8) w b) ++ "|" ++ toString(getPiece ('g',8) w b) ++ "|" ++ toString(getPiece ('h',8) w b) ++ "|")
	   putStrLn  ("7" ++ "|" ++ toString(getPiece ('a',7) w b) ++ "|" ++ toString(getPiece ('b',7) w b) ++ "|" ++ toString(getPiece ('c',7) w b) ++ "|" ++ toString(getPiece ('d',7) w b) ++ "|" ++ toString(getPiece ('e',7) w b) ++ "|" ++ toString(getPiece ('f',7) w b) ++ "|" ++ toString(getPiece ('g',7) w b) ++ "|" ++ toString(getPiece ('h',7) w b) ++ "|")
	   putStrLn  ("6" ++ "|" ++ toString(getPiece ('a',6) w b) ++ "|" ++ toString(getPiece ('b',6) w b) ++ "|" ++ toString(getPiece ('c',6) w b) ++ "|" ++ toString(getPiece ('d',6) w b) ++ "|" ++ toString(getPiece ('e',6) w b) ++ "|" ++ toString(getPiece ('f',6) w b) ++ "|" ++ toString(getPiece ('g',6) w b) ++ "|" ++ toString(getPiece ('h',6) w b) ++ "|")
	   putStrLn  ("5" ++ "|" ++ toString(getPiece ('a',5) w b) ++ "|" ++ toString(getPiece ('b',5) w b) ++ "|" ++ toString(getPiece ('c',5) w b) ++ "|" ++ toString(getPiece ('d',5) w b) ++ "|" ++ toString(getPiece ('e',5) w b) ++ "|" ++ toString(getPiece ('f',5) w b) ++ "|" ++ toString(getPiece ('g',5) w b) ++ "|" ++ toString(getPiece ('h',5) w b) ++ "|")					
	   putStrLn  ("4" ++ "|" ++ toString(getPiece ('a',4) w b) ++ "|" ++ toString(getPiece ('b',4) w b) ++ "|" ++ toString(getPiece ('c',4) w b) ++ "|" ++ toString(getPiece ('d',4) w b) ++ "|" ++ toString(getPiece ('e',4) w b) ++ "|" ++ toString(getPiece ('f',4) w b) ++ "|" ++ toString(getPiece ('g',4) w b) ++ "|" ++ toString(getPiece ('h',4) w b) ++ "|")
	   putStrLn  ("3" ++ "|" ++ toString(getPiece ('a',3) w b) ++ "|" ++ toString(getPiece ('b',3) w b) ++ "|" ++ toString(getPiece ('c',3) w b) ++ "|" ++ toString(getPiece ('d',3) w b) ++ "|" ++ toString(getPiece ('e',3) w b) ++ "|" ++ toString(getPiece ('f',3) w b) ++ "|" ++ toString(getPiece ('g',3) w b) ++ "|" ++ toString(getPiece ('h',3) w b) ++ "|")
	   putStrLn  ("2" ++ "|" ++ toString(getPiece ('a',2) w b) ++ "|" ++ toString(getPiece ('b',2) w b) ++ "|" ++ toString(getPiece ('c',2) w b) ++ "|" ++ toString(getPiece ('d',2) w b) ++ "|" ++ toString(getPiece ('e',2) w b) ++ "|" ++ toString(getPiece ('f',2) w b) ++ "|" ++ toString(getPiece ('g',2) w b) ++ "|" ++ toString(getPiece ('h',2) w b) ++ "|")
	   putStrLn  ("1" ++ "|" ++ toString(getPiece ('a',1) w b) ++ "|" ++ toString(getPiece ('b',1) w b) ++ "|" ++ toString(getPiece ('c',1) w b) ++ "|" ++ toString(getPiece ('d',1) w b) ++ "|" ++ toString(getPiece ('e',1) w b) ++ "|" ++ toString(getPiece ('f',1) w b) ++ "|" ++ toString(getPiece ('g',1) w b) ++ "|" ++ toString(getPiece ('h',1) w b) ++ "|")
	   putStrLn	 ("------------------------------------------")
	   putStr    ("Turn:"++ show color)
